...
int main(int argc, char **argv)
{
	...
	//        The terminal takes multiple steps of setup, so the final terminal is not the 
	//			child process you create.
	//        USE block_pid for this fork()
	block_pid=fork();
	/*if (block_pid < 0) {
		fprintf(stderr, "Faile to create child process.");
	}*/
	...
	if (block_pid != 0) {
		...
		execlp("/usr/bin/xfce4-terminal", "xfce4-terminal", NULL);
		//execlp("/usr/bin/gnome-terminal", "gnome-terminal", NULL);
	}
